...
@section('titulo', 'Modelos Eliminados')
...
@section('conteudo')
    <!-- Modelos Trashed Table -->
    <div class="card">
        <div class="card-datatable table-responsive">
            <div class="d-flex justify-content-between p-4 align-items-center mb-3">
                <h4 class="mb-0">Modelos Eliminados</h4>
                <div>
                    <a href="{{ route('admin.modelo.index') }}" class="btn btn-primary me-2">
                        <i class="bx bx-arrow-back me-1"></i> 
                    </a>
                    <a href="{{ route('admin.modelo.restoreAll') }}" class="btn btn-primary me-2">
                        <i class="bx bx-restore me-1"></i> 
                    </a>
                    <a href="{{ route('admin.modelo.deleteAll') }}" class="btn btn-danger" onclick="return confirm('Tem certeza que deseja excluir permanentemente todos os modelos?')">
                        <i class="bx bx-trash me-1"></i> 
                    </a>
                </div>
            </div>
            <table class="datatables-modelo-trashed table">
                ...
                <tbody>
                    @forelse($items as $item)
                        <tr>
                            ...
                            <td>{{ $item->nome }}</td>
                            <td>{{ $item->data }}</td>
                            <td>{{ $item->fk_dataset->nome_registro ?? 'N/A' }}</td>
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <a href="{{ route('admin.modelo.restore', ['id' => $item->id]) }}" class="dropdown-item">
                                                <i class="bx bx-restore me-1"></i> Restaurar
                                            </a>
                                        </li>
                                        <li>
                                            <a href="{{ route('admin.modelo.purge', ['id' => $item->id]) }}" class="dropdown-item" onclick="return confirm('Tem certeza que deseja excluir permanentemente este modelo?')">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center">Nenhum modelo eliminado encontrado.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            @if(isset($item))
                $('.datatables-modelo-trashed').DataTable({
                    responsive: true,
                    order: [[0, 'asc']],
                    language: {
