...
@section('titulo', 'Pre de Processamentos Eliminados')
...
@section('conteudo')
    <!-- Pre de Processamentos Trashed Table -->
    <div class="card">
        <div class="card-datatable table-responsive">
            <div class="d-flex justify-content-between p-4 align-items-center mb-3">
                <h4 class="mb-0">Pre de Processamentos Eliminados</h4>
                <div>
                    <a href="{{ route('admin.pre_processamento.index') }}" class="btn btn-primary me-2">
                        <i class="bx bx-arrow-back me-1"></i> 
                    </a>
                    <a href="{{ route('admin.pre_processamento.restoreAll') }}" class="btn btn-primary me-2">
                        <i class="bx bx-restore me-1"></i> 
                    </a>
                    <a href="{{ route('admin.pre_processamento.deleteAll') }}" class="btn btn-danger" onclick="return confirm('Tem certeza que deseja excluir permanentemente todos os pre_processamentoss?')">
                        <i class="bx bx-trash me-1"></i> 
                    </a>
                </div>
            </div>
            <table class="datatables-pre_processamentos-trashed table">
                ...
                <tbody>
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <a href="{{ route('admin.pre_processamento.restore', ['id' => $item->id]) }}" class="dropdown-item">
                                                <i class="bx bx-restore me-1"></i> Restaurar
                                            </a>
                                        </li>
                                        <li>
                                            <a href="{{ route('admin.pre_processamento.purge', ['id' => $item->id]) }}" class="dropdown-item" onclick="return confirm('Tem certeza que deseja excluir permanentemente este pre_processamentos?')">
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="text-center">Nenhum pre_processamentos eliminado encontrado.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            @if(isset($item))
                $('.datatables-pre_processamentos-trashed').DataTable({
                    responsive: true,
                    order: [[0, 'asc']],
                    language: {
