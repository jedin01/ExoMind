...
@section('titulo', 'Detalhes do Module')
        ...
        <div class="card-body pt-12">
          ...
          <div class="info-container">
            <ul class="list-unstyled mb-6">
              <li class="mb-2">
                <span class="h6">Prefix: </span>
                <span>{{ $item->vc_prefix }}</span>
              </li>
              <li class="mb-2">
                <span class="h6">Route_subgroup: </span>
                <span>{{ $item->vc_route_subgroup }}</span>
              </li>
              <li class="mb-2">
                <span class="h6">Url: </span>
                <span>{{ $item->vc_url }}</span>
              </li>
              <li class="mb-2">
                <span class="h6">Sub_group: </span>
                <span>{{ $item->vc_sub_group }}</span>
              </li>
              <li class="mb-2">
                <span class="h6">Icon: </span>
                <span>{{ $item->vc_icon }}</span>
              </li>
              <li class="mb-2">
                <span class="h6">Group:</span>
                <span>{{ isset($item->fk_group) ? $item->fk_group->nome_registro : '' }}</span>
              </li>
            </ul>
    ...
    <div class="col-xl-8 col-lg-7 order-0 order-md-1">
      ...
      <div class="bs-stepper wizard-numbered mt-2">
        <div class="bs-stepper-header" style="overflow-x: scroll;">
          <div class="step" data-target="#role_permissions">
            <button type="button" class="step-trigger">
              <span class="bs-stepper-circle">2</span>
              <span class="bs-stepper-label">
                <span class="bs-stepper-title">Rolepermissions</span>
              </span>
            </button>
          </div>
        </div>
        <div class="bs-stepper-content">
          <div class="card mb-6 content" id="role_permissions">
            @include('admin.role_permissions.data',[
              'items' => $item->fk_role_permissions,
              'module_detail' => $item,
              'title' => "Gestão de Rolepermissions",
              'subtitle' => "Rolepermissions",
              'relation_name' => "role_permissions",
              'adicionar' => true,
              'editar' => true,
              'remover' => true
            ])
  ...
  <div class="modal-dialog modal-xl modal-dialog-centered modal-simple">
    <div class="modal-content">
      <div class="modal-body">
        ...
        <form method="post" enctype="multipart/form-data" action="{{route('admin.module.update',['id' => $item->id])}}" class="row">
          @csrf
