...
@section('titulo', 'Modules Eliminados')
...
@section('conteudo')
    <!-- Modules Trashed Table -->
    <div class="card">
        <div class="card-datatable table-responsive">
            <div class="d-flex justify-content-between p-4 align-items-center mb-3">
                <h4 class="mb-0">Modules Eliminados</h4>
                <div>
                    <a href="{{ route('admin.module.index') }}" class="btn btn-primary me-2">
                        <i class="bx bx-arrow-back me-1"></i> 
                    </a>
                    <a href="{{ route('admin.module.restoreAll') }}" class="btn btn-primary me-2">
                        <i class="bx bx-restore me-1"></i> 
                    </a>
                    <a href="{{ route('admin.module.deleteAll') }}" class="btn btn-danger" onclick="return confirm('Tem certeza que deseja excluir permanentemente todos os modules?')">
                        <i class="bx bx-trash me-1"></i> 
                    </a>
                </div>
            </div>
            <table class="datatables-module-trashed table">
                <thead class="border-top">
                    <tr>
                        <th>ID</th>
                        <th>NOME</th>
                        <th>PREFIX</th>
                        <th>ROUTE_SUBGROUP</th>
                        <th>URL</th>
                        <th>SUB_GROUP</th>
                        <th>ICON</th>
                        <th>GROUP</th>
                ...
                <tbody>
                    @forelse($items as $item)
                        <tr>
                            <td>{{ $item->id }}</td>
                            <td>{{ $item->vc_nome }}</td>
                            <td>{{ $item->vc_prefix }}</td>
                            <td>{{ $item->vc_route_subgroup }}</td>
                            <td>{{ $item->vc_url }}</td>
                            <td>{{ $item->vc_sub_group }}</td>
                            <td>{{ $item->vc_icon }}</td>
                            <td>{{ $item->fk_group->nome_registro ?? 'N/A' }}</td>
                            <td>
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <a href="{{ route('admin.module.restore', ['id' => $item->id]) }}" class="dropdown-item">
                                                <i class="bx bx-restore me-1"></i> Restaurar
                                            </a>
                                        </li>
                                        <li>
                                            <a href="{{ route('admin.module.purge', ['id' => $item->id]) }}" class="dropdown-item" onclick="return confirm('Tem certeza que deseja excluir permanentemente este module?')">
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="text-center">Nenhum module eliminado encontrado.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            @if(isset($item))
                $('.datatables-module-trashed').DataTable({
                    responsive: true,
                    order: [[0, 'asc']],
                    language: {
