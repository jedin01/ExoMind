...
@section('titulo', 'Logs Eliminados')
    ...
    <div class="card">
        <div class="card-datatable table-responsive">
            <div class="d-flex justify-content-between p-4 align-items-center mb-3">
                <h4 class="mb-0">Logs Eliminados</h4>
                <div>
                    <a href="{{ route('admin.log.index') }}" class="btn btn-primary me-2">
                        <i class="bx bx-arrow-back me-1"></i> 
                    </a>
                    <a href="{{ route('admin.log.restoreAll') }}" class="btn btn-primary me-2">
                        <i class="bx bx-restore me-1"></i> 
                    </a>
                    <a href="{{ route('admin.log.deleteAll') }}" class="btn btn-danger" onclick="return confirm('Tem certeza que deseja excluir permanentemente todos os logs?')">
                        <i class="bx bx-trash me-1"></i> 
                    </a>
                </div>
            </div>
            <table class="datatables-log-trashed table">
                ...
                <tbody>
                    @forelse($items as $item)
                        <tr>
                            <td>{{ $item->id }}</td>
                            <td>{{ $item->lt_description }}</td>
                            <td>{{ $item->vc_agent }}</td>
                            <td>{{ $item->vc_ip_address }}</td>
                            <td>{{ $item->vc_level }}</td>
                            <td>{{ $item->fk_user->nome_registro ?? 'N/A' }}</td>
                            <td>
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <a href="{{ route('admin.log.restore', ['id' => $item->id]) }}" class="dropdown-item">
                                                <i class="bx bx-restore me-1"></i> Restaurar
                                            </a>
                                        </li>
                                        <li>
                                            <a href="{{ route('admin.log.purge', ['id' => $item->id]) }}" class="dropdown-item" onclick="return confirm('Tem certeza que deseja excluir permanentemente este log?')">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center">Nenhum log eliminado encontrado.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            @if(isset($item))
                $('.datatables-log-trashed').DataTable({
                    responsive: true,
                    order: [[0, 'asc']],
                    language: {
