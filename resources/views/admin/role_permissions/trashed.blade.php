...
@section('titulo', 'Role de Permissions Eliminados')
...
@section('conteudo')
    <!-- Role de Permissions Trashed Table -->
    <div class="card">
        <div class="card-datatable table-responsive">
            <div class="d-flex justify-content-between p-4 align-items-center mb-3">
                <h4 class="mb-0">Role de Permissions Eliminados</h4>
                <div>
                    <a href="{{ route('admin.role_permissions.index') }}" class="btn btn-primary me-2">
                        <i class="bx bx-arrow-back me-1"></i> 
                    </a>
                    <a href="{{ route('admin.role_permissions.restoreAll') }}" class="btn btn-primary me-2">
                        <i class="bx bx-restore me-1"></i> 
                    </a>
                    <a href="{{ route('admin.role_permissions.deleteAll') }}" class="btn btn-danger" onclick="return confirm('Tem certeza que deseja excluir permanentemente todos os role_permissionss?')">
                        <i class="bx bx-trash me-1"></i> 
                    </a>
                </div>
            </div>
            <table class="datatables-role_permissions-trashed table">
                ...
                <tbody>
                    @forelse($items as $item)
                        <tr>
                            <td>{{ $item->id }}</td>
                            <td>{{ $item->bl_read }}</td>
                            <td>{{ $item->bl_update }}</td>
                            <td>{{ $item->bl_create }}</td>
                            <td>{{ $item->bl_delete }}</td>
                            <td>{{ $item->fk_role->nome_registro ?? 'N/A' }}</td>
                            <td>{{ $item->fk_module->nome_registro ?? 'N/A' }}</td>
                            <td>
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <a href="{{ route('admin.role_permissions.restore', ['id' => $item->id]) }}" class="dropdown-item">
                                                <i class="bx bx-restore me-1"></i> Restaurar
                                            </a>
                                        </li>
                                        <li>
                                            <a href="{{ route('admin.role_permissions.purge', ['id' => $item->id]) }}" class="dropdown-item" onclick="return confirm('Tem certeza que deseja excluir permanentemente este role_permissions?')">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center">Nenhum role_permissions eliminado encontrado.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            @if(isset($item))
                $('.datatables-role_permissions-trashed').DataTable({
                    responsive: true,
                    order: [[0, 'asc']],
                    language: {
