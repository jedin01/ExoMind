                ...
                <div class="card ">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h4 class="mb-1">Role de Permissions</h4>
                                <p class="mb-0 opacity-75">Gerencie todos os registros de rolepermissions</p>
                            </div>
                ...
                <div class="card shadow-sm">
                    <div class="card-header border-bottom">
                        <h5 class="card-title mb-0">
                            ...
                            Lista de Role de Permissions
                        </h5>
                    </div>
                    
                    <div class="card-datatable table-responsive">
                        <table class="datatables-role_permissions table">
                                ...
                                @forelse($items as $item)
                                    <tr>
                                        <td class="text-center">
                                            ...
                                            <td>
                                                    <span class="badge bg-{{ $item->bl_read ? 'success' : 'danger' }}">
{{ $item->bl_read ? 'Sim' : 'Não' }}                                                    </span>
                                            </td>
                                            <td>
                                                    <span class="badge bg-{{ $item->bl_update ? 'success' : 'danger' }}">
{{ $item->bl_update ? 'Sim' : 'Não' }}                                                    </span>
                                            </td>
                                            <td>
                                                    <span class="badge bg-{{ $item->bl_create ? 'success' : 'danger' }}">
{{ $item->bl_create ? 'Sim' : 'Não' }}                                                    </span>
                                            </td>
                                            <td>
                                                    <span class="badge bg-{{ $item->bl_delete ? 'success' : 'danger' }}">
{{ $item->bl_delete ? 'Sim' : 'Não' }}                                                    </span>
                                            </td>
...
{{ $item->fk_module->nome ?? 'N/A' }}                                                </span>
                                        ...
                                        <td class="text-center">
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    <li>
                                                        <a 
                                                            href="{{ route('admin.role_permissions.show', ['id' => $item->id]) }}" 
                                                            class="dropdown-item"
                                                        >
                                                    ...
                                                    @if ($editar)
                                                        <li>
                                                            <a 
                                                                ...
                                                                class="dropdown-item" 
                                                                data-bs-toggle="modal" 
                                                                data-bs-target="#editRolePermissionsModal--{{ $item->id }}"
                                                            >
                                                    ...
                                                    @if ($remover)
                                                        ...
                                                        <li>
                                                            <a 
                                                                href="#" 
                                                                data-url="{{ route('admin.role_permissions.destroy', ['id' => $item->id]) }}" 
                                    ...
                                    @if ($editar)
                                        <div class="modal fade" id="editRolePermissionsModal--{{ $item->id }}" tabindex="-1" aria-hidden="true">
                                            <div class="modal-dialog modal-lg modal-dialog-centered">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title">Editar RolePermissions</h5>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                    </div>
                                                    <div class="modal-body">
                                                        <form 
                                                            method="post" 
                                                            action="{{ route('admin.role_permissions.update', ['id' => $item->id]) }}" 
                                                            class="row g-3" 
                                                        ...
                                                        >
                                                            ...
                                                            @include('forms.role_permissions.index')
                                    ...
                                    <tr>
                                        <td colspan="8" class="text-center py-5">
                                            <div class="d-flex flex-column align-items-center">
                                                ...
                                                <p class="text-muted mb-0">Não há rolepermissions cadastrados no momento.</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
        ...
        @if ($adicionar)
            <div class="modal fade" id="addRolePermissionsModal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-lg modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Adicionar Novo RolePermissions</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        ...
                        <div class="modal-body">
                            <form 
                                class="row g-3" 
                                action="{{ route('admin.role_permissions.store') }}" 
                                method="post" 
                                enctype="multipart/form-data"
                            >
                                @csrf
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            $('.datatables-role_permissions').DataTable({
                responsive: {
                    details: {
                        type: 'column',
                        target: 'tr'
