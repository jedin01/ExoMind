...
@section('titulo', 'Treinamento de Metricas Eliminados')
...
@section('conteudo')
    <!-- Treinamento de Metricas Trashed Table -->
    <div class="card">
        <div class="card-datatable table-responsive">
            <div class="d-flex justify-content-between p-4 align-items-center mb-3">
                <h4 class="mb-0">Treinamento de Metricas Eliminados</h4>
                <div>
                    <a href="{{ route('admin.treinamento_metrica.index') }}" class="btn btn-primary me-2">
                        <i class="bx bx-arrow-back me-1"></i> 
                    </a>
                    <a href="{{ route('admin.treinamento_metrica.restoreAll') }}" class="btn btn-primary me-2">
                        <i class="bx bx-restore me-1"></i> 
                    </a>
                    <a href="{{ route('admin.treinamento_metrica.deleteAll') }}" class="btn btn-danger" onclick="return confirm('Tem certeza que deseja excluir permanentemente todos os treinamento_metricas?')">
                        <i class="bx bx-trash me-1"></i> 
                    </a>
                </div>
            </div>
            <table class="datatables-treinamento_metrica-trashed table">
                ...
                <tbody>
                    @forelse($items as $item)
                        <tr>
                            <td>{{ $item->id }}</td>
                            <td>{{ $item->fk_metricas->nome_registro ?? 'N/A' }}</td>
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <a href="{{ route('admin.treinamento_metrica.restore', ['id' => $item->id]) }}" class="dropdown-item">
                                                <i class="bx bx-restore me-1"></i> Restaurar
                                            </a>
                                        </li>
                                        <li>
                                            <a href="{{ route('admin.treinamento_metrica.purge', ['id' => $item->id]) }}" class="dropdown-item" onclick="return confirm('Tem certeza que deseja excluir permanentemente este treinamento_metrica?')">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="text-center">Nenhum treinamento_metrica eliminado encontrado.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            @if(isset($item))
                $('.datatables-treinamento_metrica-trashed').DataTable({
                    responsive: true,
                    order: [[0, 'asc']],
                    language: {
